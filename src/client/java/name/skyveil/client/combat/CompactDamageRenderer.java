...
public final class CompactDamageRenderer {
    ...
    private static void render(LevelRenderContext context){
        var camera=context.levelState().cameraRenderState;if(camera==null||camera.pos==null)return;Minecraft client=Minecraft.getInstance();if(client.level==null)return;
        for(DamageBatcher.Display display:CompactDamageManager.displays()){
            ...
            PoseStack pose=context.poseStack();pose.pushPose();pose.translate(position.x-camera.pos.x,position.y-camera.pos.y,position.z-camera.pos.z);
            Component label=Component.literal(DamageTextParser.format(display.average())).withStyle(ChatFormatting.BOLD,ChatFormatting.GOLD);
            context.submitNodeCollector().submitNameTag(pose,new Vec3(0,target.getBbHeight()+1.15,0),0,label,true,0xF000F0,distance,context.levelState().cameraRenderState);pose.popPose();
        }
    }
}
