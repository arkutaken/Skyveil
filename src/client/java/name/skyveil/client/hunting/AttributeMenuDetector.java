...
/** Exact live title plus server-container geometry recognition for Hypixel's Attribute Menu. */
public final class AttributeMenuDetector {
    ...
    public static Page page(AbstractContainerScreen<?> screen){
        return parsePage(screen==null?"":screen.getTitle().getString());
    }
    static Page parsePage(String title){
        Matcher matcher=TITLE.matcher(normalize(title));
        if(!matcher.matches()||matcher.group(1)==null)return new Page(1,1);
        try{return new Page(Integer.parseInt(matcher.group(1)),Integer.parseInt(matcher.group(2)));}catch(NumberFormatException ignored){return new Page(1,1);}
    }
