...
public final class HuntingBoxValuePanel {
    ...
    public static void render(AbstractContainerScreen<?> screen,GuiGraphicsExtractor graphics,int guiLeft,int guiTop,int guiWidth,int guiHeight,int mouseX,int mouseY){
        ...
        }finally{graphics.disableScissor();}graphics.text(Minecraft.getInstance().font,"Hypixel Bazaar • "+priceStatus.name().toLowerCase(Locale.ROOT),x+5,y+height-12,SkyveilTheme.SECONDARY,false);
    }
    ...
    public static void reset(){cachedScreen=null;dirty=true;pendingFingerprint=Integer.MIN_VALUE;pendingConfirmations=0;scroll=0;totalPages=1;ENTRIES.clear();PAGE_KEYS.clear();VISITED.clear();cachedRows=List.of();entryRevision++;cachedEntryRevision=-1;cachedPriceRevision=-1;bounds=null;}
    ...
    private static void scan(AbstractContainerScreen<?> screen){
        ...
        Set<String> previous=PAGE_KEYS.put(page.current(),Set.copyOf(currentKeys));if(previous!=null)for(String key:previous)if(!currentKeys.contains(key))ENTRIES.remove(key);VISITED.add(page.current());entryRevision++;AttributeSessionData.observeOwnedPage(page.current(),attributeQuantities);ShardPriceService.ensureFresh();
    }
    private static List<Row> rows(){long priceRevision=ShardPriceService.revision();if(entryRevision==cachedEntryRevision&&priceRevision==cachedPriceRevision)return cachedRows;ArrayList<Row> rows=new ArrayList<>();for(Entry entry:ENTRIES.values())if(entry.amount()>0){var quote=entry.identity()==null?null:ShardPriceService.quote(entry.identity().bazaarName());rows.add(new Row(entry.stack(),entry.amount(),quote==null?null:quote.instantSell()));}rows.sort(Comparator.comparingLong((Row row)->row.unitPrice()==null?Long.MIN_VALUE:safeMultiply(row.unitPrice(),row.amount())).reversed().thenComparing(row->row.stack().getHoverName().getString(),String.CASE_INSENSITIVE_ORDER));cachedRows=List.copyOf(rows);cachedEntryRevision=entryRevision;cachedPriceRevision=priceRevision;return cachedRows;}
